...
parameter_n_estimators = st.sidebar.slider('Number of estimators (n_estimators)', 10, 500, 50)
parameter_n_estimators_step = st.sidebar.number_input('Step size for n_estimators', 10)
st.sidebar.write('---')
parameter_max_features = st.sidebar.slider('Max features (max_features)', 1, 50, 3)
...
def build_model(df):
    ...
    grid = GridSearchCV(estimator=rf, param_grid={'max_features': [1, 2, 3], 'n_estimators': [10, 50, 100]}, cv=5)
    ...
    # Accessing the columns and index directly
...
if uploaded_file is not None:
    ...
    if df.isnull().values.any():
        st.error("Uploaded data contains missing values. Please clean your dataset.")
    elif df.shape[1] < 2:
        st.error("Uploaded data must have at least two columns.")
    else:
        st.write(df)
        build_model(df)
else:
